...
Hosted on GitHub Pages from `main` / `/ (root)`. These files have to stay at the repository root, because Pages only serves from `/` or `/docs`, so moving them into a subfolder breaks the site. Push to `main` and Pages rebuilds on its own.
